  ...
  <div class="row text-right">
    {{ Form::open(array('role' => 'form', 'class' => 'form-inline', 'method' => 'get')) }}
      <div class="form-group">
        ...
        {{ Form::text('source', '', array('class' => 'form-control', 'id' => 'inputTel', 'placeholder' => '原始编号')) }}
      </div>
      <div class="form-group">
  ...
  <div class="row">
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr>
            <td><label>{{ Form::checkbox('question_id[]', $list->question->id) }} {{ $list->question->id }}</label></td>
            <td>{{$list->question->txt}}</td>
            <td>{{$list->question->source}}</td>
            <td>{{$statusEnum[$list->question->status]}}</td>
            <td>{{$list->question->created_at}}</td>
            <td>
              <div class="btn-group btn-xs">
                  <a class="btn btn-default btn-xs" href="{{url('/admin/topic/edit?id='. $list->question->id) }}"><i class="icon-edit"></i> 编辑</a>
                  <a class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown" href="#"><span class="icon-caret-down"></span></a>
                  <ul class="dropdown-menu">
                      @if($list->question->status === 1)
                      <li><a style='color:#999;'><i class="icon-ok"></i> 有效</a></li>
                      <li><a href="#" class="btn_publish" data-toggle="modal" data-id="{{$list->question->id}}" data-val="{{$list->question->id}}" data-status="0"><i class="icon-remove"></i> 无效</a></li>
                      <li><a href="#" class="btn_publish" data-toggle="modal" data-id="{{$list->question->id}}" data-val="{{$list->question->id}}" data-status="-1"><i class="icon-remove"></i> 审核未通过</a></li>
                      @elseif($list->question->status === 0)
                      <li><a href="#" class="btn_publish" data-toggle="modal" data-id="{{$list->question->id}}" data-val="{{$list->question->id}}" data-status="1"><i class="icon-ok"></i> 有效</a></li>
                      ...
                      @endif
                      <li class="divider"></li>
                      <li><a href="#" class="btn_delete" data-toggle="modal" data-id="{{$list->question->id}}" data-val="{{$list->question->id}}" data-status="1"><i class="icon-trash"></i> 删除</a></li>
                  </ul>
              </div>
            </td>
          </tr>
