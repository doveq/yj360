...
@section('content')
  <div class="row">
    <ol class="breadcrumb">
      ...
      @if ($query['parent_id'] > 0)
      <a href="{{url('/admin/examSort?parent_id='.$parent->parent_id)}}"><span class='pull-right  icon-arrow-up'> 返回上级</span></a>
      @endif
    </ol>
  </div>
  ...
  <div class="row">
      <div class="col-md-4 col-md-offset-8 text-right">
        @if(count($lists) > 0)
        {{ link_to_route('admin.examSort.create', ' 添加同级分类', array('parent_id' => $query['parent_id']), array('class' => 'btn btn-primary')) }}
        @else
          {{ link_to_route('admin.examSort.create', ' 添加同级分类', array('parent_id' => $query['parent_id']), array('class' => 'btn btn-primary')) }}
        @endif
      </div>
      ...
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr>
            <td>{{$list->id}}</td>
            <td><a href="{{url('/admin/examSort?parent_id='. $list->id) }}">{{$list->name}}</a></td>
            ...
            <td>
              <div class="btn-group btn-xs">
                <a class="btn btn-default btn-xs" href="{{url('/admin/examSort/'. $list->id .'/edit') }}"><i class="icon-edit"></i> 编辑</a>
...
$(function(){
  ...
  $(".btn_publish").bind("click", function(){
      ...
      $("#myModalForm").attr('action', '{{ url('/admin/examSort/') }}/' + column_id);
  ...
  $(".btn_delete").bind("click", function(){
      ...
      $("#column_id").val(column_id);
      $("#myModalForm").attr('action', '/admin/examSort/'+ column_id);
